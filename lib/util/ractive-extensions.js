...
// (function() {
// 	require(['controllers/home-controller', 'controllers/shop/shop-cart-controller'], function(HomeController, ShopCartController) {
// 		return {
// 			'home-controller': HomeController,
// 			'shop-shop-cart-controller': ShopCartController
// 		};
// 	});
// }());

var jsBundleTemplate2 = "(function() {" +
...
function createJsBundle(bundleName, html, isAjaxRequest, done) {
	var $ = cheerio.load(html),
		_bundleName = isAjaxRequest ? bundleName + '_xhr' : bundleName,
		dependencies = [], args = [], result = [],
		$controllers, bundle, controllerName, controllerType;
	...
	$controllers.each(function(index, controller) {
		...
		console.log('Controlador:', controllerName, '- Tipo:', controllerType);
		if (!isAjaxRequest || !controllerType) {
			dependencies.push("'controllers/" + controllerName + "'");
			args.push(controllerName.replace(/-/g, ''));
			result.push("'" + controllerName + "':" + controllerName.replace(/-/g, ''));
		}
	});

	bundle = jsBundleTemplate2.replace(/dependencies/, dependencies.join(','));
	bundle = bundle.replace(/args/, args.join(','));
	bundle = bundle.replace(/result/, result.join(','));

	fs.writeFile(process.cwd() + '/public/js/bundles/' + _bundleName + '.js',
		...
		function(err) {
			...
			console.log('Ractive-extensions::createJsBundle# JS bundle created:', bundleName);
			done();
		});
}

function render(str, options, done) {
	var view = this;
	...
	async.waterfall([
		...
		function createJsBundles(html, next) {
			// TODO Only in development environments
			createJsBundle(view.name, html, options.xhr, function(err) {
				if (err) {
